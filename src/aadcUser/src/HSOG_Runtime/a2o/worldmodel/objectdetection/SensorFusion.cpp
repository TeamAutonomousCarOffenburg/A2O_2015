...
SensorFusion::SensorFusion(IResource* resource)
        :_resource(resource){}
...
void SensorFusion::run()
{
  while(!m_stop){
    ...
    std::vector<IVisibleObject::Ptr> resultDistance = getObjectsFromDistanceSensors(distanceData, _recognizedBoxes);

    std::vector<IVisibleObject::Ptr> result = resultCamera;
    
    // get rid of double objects
    std::vector<IVisibleObject::Ptr> independentObj;
    for(IVisibleObject::Ptr dObj : resultDistance)
    {
      bool match = false;
      for(IVisibleObject::Ptr cObj : resultCamera)
      {
...
std::vector< IVisibleObject::Ptr > SensorFusion::getObjectsFromCam(const std::vector< AlignedBoundingBox3D::Ptr >& cameraData, 
  ...
  if(cameraData.size() > 0)
  {
	...
	*box3d += CYCLES_PER_SECOND;
	knownBoxes.push_back(box3d);
      ...
      // check if road sign
      if((((xDiff > 0.008 && xDiff < 0.036) && 
	(yDiff > 0.12 && yDiff < 0.16)) ||
	((yDiff > 0.008 && yDiff < 0.036) && 
	(xDiff > 0.12 && xDiff < 0.16)))
	&& (zDiff > 0.23 && zDiff < 0.25))
      {
	object = boost::make_shared<RoadSign>(RoadSign(SignType::Unknown , Pose2D(box3d->origin()(0), box3d->origin()(1))));
      }
      else
      {
	object = boost::make_shared<VisibleObject>(VisibleObject("Object", Pose2D(box3d->origin()(0), box3d->origin()(1)), polygon));
      }
...
std::vector< IVisibleObject::Ptr > SensorFusion::getObjectsFromDistanceSensors(const std::vector< AlignedBoundingBox3D::Ptr >& distanceData, 
		std::vector<AlignedBoundingBox3D::Ptr>& knownBoxes )
{
  ...
  if(distanceData.size() > 0)
  {
	...
	*box3d += CYCLES_PER_SECOND;
	knownBoxes.push_back(box3d);
      ...
      IVisibleObject::Ptr object = IVisibleObject::Ptr(new VisibleObject("Object", 
			    Pose2D(box3d->origin()(0), box3d->origin()(1)), polygon));
