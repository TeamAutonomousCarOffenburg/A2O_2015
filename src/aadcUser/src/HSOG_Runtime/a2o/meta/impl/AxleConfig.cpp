...
AxleConfig::AxleConfig(const std::string& name,
		       const Eigen::Vector3d position,
		       const double& length,
		       const double& wheelDiameter,
		       const Angle& wheelAngle,
		       const std::string& leftWheelTacho,
		       const std::string& rightWheelTacho)
      : _name(name),
	_position(position(0), 0, position(2)),
	_length(length),
	_wheelDiameter(wheelDiameter),
	_wheelAngle(wheelAngle),
	_leftWheelTacho(leftWheelTacho),
	_rightWheelTacho(rightWheelTacho)
{
}
