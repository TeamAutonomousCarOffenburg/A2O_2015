...
template<typename T>
bool FuzzyCompare::eq(const T& lhs, const T& rhs, const T& range)
{
  if (lhs > rhs) {
...
template<typename T>
bool FuzzyCompare::gte(const T& lhs, const T& rhs, const T& range)
{
  return lhs >= (rhs - range);
}

template<typename T>
bool FuzzyCompare::lte(const T& lhs, const T& rhs, const T& range)
{
  return lhs <= (rhs + range);
}
 ...
 *         values
 */
double FuzzyCompare::getLinearFuzzyValue(double x0, double x1,
		bool ascending, double x)
{
	if (x0 > x1) {
		double temp = x0;
		x0 = x1;
 ...
 *         passed points
 */
double FuzzyCompare::linearInterpolation(double x0, double y0, double x1,
			double y1, double x)
{
