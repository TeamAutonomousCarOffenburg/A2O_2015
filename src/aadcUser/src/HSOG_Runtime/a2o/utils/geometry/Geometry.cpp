...
const double Geometry::getDistance(const double& p1x,
				   const double& p1y,
				   const double& p2x,
				   const double& p2y)
{
...
const bool Geometry::checkParallel(const Line2D& line1,
				   const Line2D& line2)
{
...
const bool Geometry::intersectionPoint(const Line2D& line1,
				       const Line2D& line2,
				       Vector2d& result)
{
...
void Geometry::intersectionPoints(const Line2D& line,
...
void Geometry::commonTangents(const Circle2D& circle1,
...
void Geometry::slope(const Circle2D& circle,
		     ...
		     Angle& result)
{
  ...
  double slope = -0.5 * sqrt(1 / (pow(circle.radius(), 2) - pow(point(0) - circle.origin()(0), 2))) * (2 * point(0) - 2 * circle.origin()(0));
...
Pose2D Geometry::calculateArcPose(const double& length,
...
void Geometry::interpolateArcPoses(const double& length,
				   const double& exitAngle,
				   ...
				   const Pose2D& startPose)
{
  if (exitAngle == 0) {
    ...
    for (size_t i = 1; i <= howMany; i++) {
      runningAngle = step * i;
      pose = Pose2D(sin(runningAngle) * radius,
		    radius * (1 - cos(runningAngle)),
		    Angle::rad(runningAngle));
      result.push_back(startPose * pose);
    }
  }
}

double Geometry::constructDoubleArc(const Pose2D& start,
				    const Pose2D& end,
...
const double Geometry::getDistanceToLine(const Vector2d& lineStart,
...
const bool Geometry::isInsidePolygon(const std::vector<Vector2d>& polygon,
				     const Vector2d& point)
{
...
const double Geometry::getDistance(const std::vector<Vector2d>& polygon,
				   const Vector2d& point)
{
  size_t size = polygon.size();
  if (size == 0) {
...
const Line2D Geometry::getClosestPolyLine(const std::vector<Vector2d>& polygon,
					  const Vector2d& point)
{
  size_t size = polygon.size();
  if (size <= 1) {
...
const double Geometry::getAvgDistance(const std::vector<Vector2d>& polygon1,
				      const std::vector<Vector2d>& polygon2)
{
