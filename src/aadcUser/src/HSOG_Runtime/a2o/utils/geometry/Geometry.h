...
public:
  ...
  static const double getDistance(const double& p1x,
				  const double& p1y,
				  const double& p2x,
				  const double& p2y);
  ...
  static const double getDistance(const Eigen::Matrix<T, Size, 1>& v1,
				  const Eigen::Matrix<T, Size, 1>& v2)
  {
  ...
  template<typename T, int Size>
  static const double getNorm(const Eigen::Matrix<T, Size, 1>& vector) 
  {
  ...
  template<typename T, int Size>
  static const Eigen::Matrix<T, Size, 1> avg(const Eigen::Matrix<T, Size, 1>& p1,
					     const Eigen::Matrix<T, Size, 1>& p2,
					     const T& weight1 = 1,
					     const T& weight2 = 1) 
  {
    T avg[Size];
  ...
  /** Calculate the bounding box of an point set.
    ...
    */
  template<typename T, int Size>
  static const std::pair<Eigen::Matrix<T, Size, 1>, Eigen::Matrix<T, Size, 1>> getBounds(const std::vector<Eigen::Matrix<T, Size, 1>>& points) 
  {
  ...
  /** Retrieve the point with the minimum value in the specified dimension.
    ...
    */
  template<typename T, int Size>
  static const Eigen::Matrix<T, Size, 1> min(const std::vector<Eigen::Matrix<T, Size, 1>>& points,
  ...
  /** Retrieve the point with the minimum value in the specified dimension.
    ...
    */
  template<typename T, int Size>
  static const Eigen::Matrix<T, Size, 1> max(const std::vector<Eigen::Matrix<T, Size, 1>>& points,
					     const unsigned int& dimension) 
  {
  ...
  /** Retrieve the point with the minimum/maximum value in the specified dimension.
    ...
    */
  template<typename T, int Size>
  static const Eigen::Matrix<T, Size, 1> minmax(const std::vector<Eigen::Matrix<T, Size, 1>>& points,
						const unsigned int& dimension,
						const bool& min) 
  {
    if (points.size() == 0) {
  ...
  static const bool checkParallel(const Line2D& line1,
				  const Line2D& line2);
  ...
  /** Calculate the intersection point of the two lines.
    ...
    */
  static const bool intersectionPoint(const Line2D& line1,
				      const Line2D& line2,
  ...
  /** Caclulate the intersection points of the given lince and circle. 
    ...
    */
  static void intersectionPoints(const Line2D& line,
  ...
  /** Caclulate the intersection points of the two circles. 
    ...
    * \param result - the vector to store the intersection points if existing
    */
  static void intersectionPoints(const Circle2D& circle1,
  ...
  /** Calculate the common tangents between two circles.
    ...
    */
  static void commonTangents(const Circle2D& circle1,
			     const Circle2D& circle2,
  ...
  /** Calculate the slope on given point.
    ...
    * 
    */
  static void slope(const Circle2D& circle,
  ...
  static Pose2D calculateArcPose(const double& length,
  ...
  /** Interpolate a bunch of intermediate poses on an arc.
    ...
    */
  static void interpolateArcPoses(const double& length,
				  const double& exitAngle,
				  const size_t& howMany,
				  std::vector<Pose2D>& result,
				  const Pose2D& startPose = Pose2D());
  
  static double constructDoubleArc(const Pose2D& start,
				   const Pose2D& end,
				   std::vector<LineSegment>& result);
  
  static const double getDistanceToLine(const Line2D& line,
					const Eigen::Vector2d& point);
  
  static const double getDistanceToLine(const Eigen::Vector2d& lineStart,
  ...
  /**  Checks if the passed point is inside the passed polygon
    ...
    */
  static const bool isInsidePolygon(const std::vector<Eigen::Vector2d>& polygon,
				    const Eigen::Vector2d& point);
  
  static const double getDistance(const std::vector<Eigen::Vector2d>& polygon,
				  const Eigen::Vector2d& point);
  
  static const Line2D getClosestPolyLine(const std::vector<Eigen::Vector2d>& polygon,
					 const Eigen::Vector2d& point);
  
  static const double getAvgDistance(const std::vector<Eigen::Vector2d>& polygon1,
				     const std::vector<Eigen::Vector2d>& polygon2);
