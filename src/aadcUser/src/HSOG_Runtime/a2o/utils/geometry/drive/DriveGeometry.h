...
public:
  /** \brief Calculate the driving pose based on the rear wheel spacing and their rolling-distances.
    * 
    * Calculate the pose of the (center of the) rear axle after rolling leftWheelDistance and rightWheelDistance on the wheels.
    * In order to to so, the wheel spacing (distance between the wheels along the axle).
    * 
    ...
    */
  static const Pose2D calculateCurvePose(const double& leftWheelDistance,
					 const double& rightWheelDistance,
					 const double& wheelSpacing);
 ...
 */
  ...
  /** Returns a curve from two poses in intervall 0, [1/2*pi, 3/2*pi]
    ...
    * \param other other pose on cosine in intervall 0.5*pi , 3/2*pi or -0.5*pi , -3/2*pi: CAUTION: angle of "other" is inaccurate if ratio != 1
    */
