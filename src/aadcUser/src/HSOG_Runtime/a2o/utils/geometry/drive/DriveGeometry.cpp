...
const Pose2D DriveGeometry::calculateCurvePose(const double& leftWheelDistance,
					  ...
					  const double& wheelSpacing)
{
  ...
  if (leftWheelDistance == rightWheelDistance) {
    ...
    if(leftWheelDistance * rightWheelDistance >= 0) {
      // Both wheels spin in the same direction, thus the origin of the driving circle is outside (left or right) the car.
...
//     return Driveable::Invalid();
  ...
  // if radius is too small  
    ...
    return Driveable(IManeuverGeometry::Ptr(new Line2D(carPose.getPosition(), lineEnd)), 
		     IManeuverGeometry::Ptr(new Circle2D(circle)), 
		     intercept);
}
...
//     return Driveable::Invalid();
  ...
  // if radius is too small
    ...
    return Driveable(IManeuverGeometry::Ptr(new Circle2D(circle)), 
		     IManeuverGeometry::Ptr(new Line2D (lineStart, nextWayPoint.getPosition())), 
		     intercept);
}
...
const Driveable DriveGeometry::getSCurveBeforeLineGeometry(const Pose2D& carPose, const Pose2D& nextWayPoint)
{
  ...
  return Driveable(IManeuverGeometry::Ptr(new Cosine(geometry)), 
		   IManeuverGeometry::Ptr(new Line2D(line)), 
