...
LineSegment::LineSegment(const Pose2D& startPose,
			 const double& length,
...
LineSegment::LineSegment(const Circle2D& circle,
			 const Vector2d& startPos,
			 const Vector2d& endPos,
			 const bool& clockwise)
      : LineSegment(circle,
		    Angle::to(startPos - circle.origin()),
		    Angle::to(endPos - circle.origin()),
		    clockwise)
{
}
			 ...
			 const bool& clockwise)
{
  ...
  _startPose = Pose2D(circle.getPointOnCircle(startAngle), startAngle + (clockwise ? Angle::Deg_N90() : Angle::Deg_90()));
  _endPose = _startPose * Geometry::calculateArcPose(_length, _exitAngle);
}
...
void LineSegment::interpolateIntermediatePoses(const size_t& howMany,
					       std::vector<Pose2D>& result) const
{
...
const double LineSegment::getExtension(const Vector2d& position) const
{
  if (std::fabs(_exitAngle) < 0.000001) {
    ...
    Pose2D circlePose = _startPose * Pose2D(0, radius, leftCurve ? Angle::deg(-90.0000001) : Angle::deg(90.0000001));
...
namespace A2O {
	std::ostream& operator<<(std::ostream& os, const A2O::LineSegment& segment)
	{
	  os << "l: " << segment.getLength() << " e-a: " << segment.getExitAngle() << " from " << segment.getStartPose() << " to " << segment.getEndPose();
	  return os;
	}
}
