...
Pose3D::Pose3D(const Vector3d& position)
      : _position(position)
{
...
Pose3D::Pose3D(const Quaterniond& orientation)
      : _position(0, 0, 0), _orientation(orientation)
{
}
...
Pose3D::Pose3D(const AngleAxisd& orientation)
      : _position(0, 0, 0), _orientation(orientation)
{
}

Pose3D::Pose3D(const Vector3d& position, const AngleAxisd& orientation)
      : _position(position), _orientation(orientation)
{
}
...
{
  const AlignedBoundingBox3D transformedBox(*this * box.origin(), _orientation * box.minPoint(), _orientation * box.maxPoint() );
