...
AccelerometerDecoder::AccelerometerDecoder(ISensorConfig::ConstPtr config,
		IEventLogger::ConstPtr logger)
      : Decoder(logger), _perceptiorName(config->getPerceptorName()), _time(0)
{
  _pins.push_back(InputPin(_perceptiorName + "_vec_x", "tSignalValue"));
  _pins.push_back(InputPin(_perceptiorName + "_vec_y", "tSignalValue"));
  _pins.push_back(InputPin(_perceptiorName + "_vec_z", "tSignalValue"));
}
...
std::vector<IPerceptor::ConstPtr> AccelerometerDecoder::decode(const InputPin& pin,
							       ...
							       adtf::IMediaSample* mediaSample)
{
  ...
  if (pinIndex >= 0) {
    ...
    if (allPinsReceived()) {
      ...
      perceptors.push_back(boost::make_shared<AccelerometerPerceptor>(_perceptiorName, _time, _x, _y, _z));
      clearReceived();
