...
public:
  ValueGroupDecoder(const std::vector<ISensorConfig::ConstPtr>& configs, IEventLogger::ConstPtr logger, double conversionFactor = 1);
  ValueGroupDecoder(const std::vector<IDistanceSensorConfig::ConstPtr>& configs, IEventLogger::ConstPtr logger, double conversionFactor = 1);
  ValueGroupDecoder(const std::vector<IActuatorConfig::ConstPtr>& configs, IEventLogger::ConstPtr logger, double conversionFactor = 1);
  ValueGroupDecoder(const std::vector<IServoDriveConfig::ConstPtr>& configs, IEventLogger::ConstPtr logger, double conversionFactor = 1);
