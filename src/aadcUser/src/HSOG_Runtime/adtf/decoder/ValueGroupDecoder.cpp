...
ValueGroupDecoder::ValueGroupDecoder(const std::vector<ISensorConfig::ConstPtr>& configs, IEventLogger::ConstPtr logger, double conversionFactor)
...
ValueGroupDecoder::ValueGroupDecoder(const vector<IDistanceSensorConfig::ConstPtr>& configs,
				     IEventLogger::ConstPtr logger, double conversionFactor)
      : Decoder(logger), _time(0), _conversionFactor(conversionFactor)
{
  for (unsigned int i = 0; i < configs.size(); i++) {
    _pins.push_back(InputPin(configs[i]->getPerceptorName(), "tSignalValue"));
...
ValueGroupDecoder::ValueGroupDecoder(const std::vector<IActuatorConfig::ConstPtr>& configs, IEventLogger::ConstPtr logger, double conversionFactor)
      : Decoder(logger), _time(0), _conversionFactor(conversionFactor)
{
  for (unsigned int i = 0; i < configs.size(); i++) {
    _pins.push_back(InputPin(configs[i]->getPerceptorName(), "tSignalValue"));
...
ValueGroupDecoder::ValueGroupDecoder(const std::vector<IServoDriveConfig::ConstPtr>& configs, IEventLogger::ConstPtr logger, double conversionFactor)
      : Decoder(logger), _time(0), _conversionFactor(conversionFactor)
{
  for (unsigned int i = 0; i < configs.size(); i++) {
    _pins.push_back(InputPin(configs[i]->getPerceptorName(), "tSignalValue"));
...
std::vector<IPerceptor::ConstPtr> ValueGroupDecoder::decode(const InputPin& pin,
