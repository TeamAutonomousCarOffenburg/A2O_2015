...
{
  _pins.push_back(InputPin(_perceptiorName + "_quat_w", "tSignalValue"));
  _pins.push_back(InputPin(_perceptiorName + "_quat_x", "tSignalValue"));
  _pins.push_back(InputPin(_perceptiorName + "_quat_y", "tSignalValue"));
  _pins.push_back(InputPin(_perceptiorName + "_quat_z", "tSignalValue"));
}
...
std::vector<IPerceptor::ConstPtr> GyroDecoder::decode(const InputPin& pin,
						      ...
						      adtf::IMediaSample* mediaSample)
{
  ...
  if (pinIndex >= 0) {
    ...
    if (allPinsReceived()) {
      ...
      perceptors.push_back(boost::make_shared<GyroPerceptor>(_perceptiorName, _time, _w, _x, _y, _z));
      clearReceived();
