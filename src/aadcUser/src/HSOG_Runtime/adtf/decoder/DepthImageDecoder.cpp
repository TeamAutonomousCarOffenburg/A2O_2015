...
DepthImageDecoder::DepthImageDecoder(ICameraConfig::ConstPtr config, IEventLogger::ConstPtr logger)
      : Decoder(logger), _perceptiorName(config->getPerceptorName()), _time(0), _frameWidth(config->getFrameWidth()), _frameHeight(config->getFrameHeight()), _focalLengthX(config->getFocalLengthX()), _focalLengthY(config->getFocalLengthY())
{
...
std::vector<IPerceptor::ConstPtr> DepthImageDecoder::decode(const InputPin& pin,
							    ...
							    adtf::IMediaSample* mediaSample)
{
  ...
  if (pinIndex >= 0) {
    ...
    if(l_pSrcBuffer)
    {
      ...
      perceptors.push_back(boost::make_shared<PointCloudPerceptor>(_perceptiorName, _time, cloud));
