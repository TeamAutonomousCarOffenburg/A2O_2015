...
CameraDecoder::CameraDecoder(ICameraConfig::ConstPtr config,
		IEventLogger::ConstPtr logger)
      : Decoder(logger), _perceptiorName(config->getPerceptorName()), _time(0), _frameWidth(config->getFrameWidth()), _frameHeight(config->getFrameHeight())
{
...
std::vector<IPerceptor::ConstPtr> CameraDecoder::decode(const InputPin& pin,
							     ...
							     adtf::IMediaSample* mediaSample)
{
  ...
  if (pinIndex >= 0) {
	...
	perceptors.push_back(boost::make_shared<CameraPerceptor>(_perceptiorName, _time, img));
