...
ADTF_FILTER_PLUGIN("HSOG rateLimiter", OID_ADTF_HSOG_RATE, rateLimiter);

rateLimiter::~rateLimiter() {}

rateLimiter::rateLimiter(const tChar* __info) : cFilter(__info) {

	A2O::IComponentFactory::ConstPtr factory(new A2O::ComponentFactory());

  	A2O::A2ORuntime* _A2ORuntime = new A2O::A2ORuntime(factory);
	_A2ORuntime->init();

	A2O::ICarMetaModel::Ptr carMetaModel = _A2ORuntime->getCarMetaModel();
	A2O::IAction::Ptr action = _A2ORuntime->getAction();

	A2O::ADTFPinMessageEncoder::Ptr encoder = A2O::ADTFPinMessageEncoder::Ptr(new A2O::ADTFPinMessageEncoder(action, carMetaModel));
...
tResult rateLimiter::CreatePins(__exception) {
	...
	for(auto it = _carOutputPins.begin(); it != _carOutputPins.end(); ++it, index++) {
		...
		cout << "Creating input and output pin " << inputname << " with type " << sensorSignalType << endl;

		cOutputPin* outputPin = new cOutputPin();
		RETURN_IF_FAILED(((cOutputPin*)outputPin)->Create(outputname.c_str(),
		...
		RETURN_IF_FAILED(((cInputPin*)inputPin)->Create(inputname.c_str(),
					new cMediaType(0, 0, 0, sensorSignalType.c_str()),
					static_cast<IPinEventSink*>(this)));
...
tResult rateLimiter::OnPinEvent(IPin* pSource, tInt nEventCode, tInt nParam1, tInt nParam2, IMediaSample* pMediaSample)
{
	if (nEventCode == IPinEventSink::PE_MediaSampleReceived) {
		A2O::OutputPin pin = _inputPinMap.at(pSource);
		...
		if(signalType=="tSignalValue")
		{
			...
			if(found != _tsignalLastValue.end())
			{
				...
				{
					cout << pin.name << " from " << oldvalue << " to " << value << endl;
					changed_enough = true;
		...
		else if(signalType=="tDriverStruct")
		{
			...
			if(found != _driverStructLastValue.end())
			{
				...
				{
					// tInt8 print weirdly
					cout << pin.name << " from state: " << (int) lastValue.first << " maneuver: " << lastValue.second << " to state: " << (int) stateId << " maneuver:  " << maneuverId << endl;
					changed_enough = true;
...
bool rateLimiter::longEnoughSinceLastSent(IPin *pSource)
{
...
tResult rateLimiter::CreateDescriptors(){
	_runtime->GetObject(OID_ADTF_MEDIA_DESCRIPTION_MANAGER, IID_ADTF_MEDIA_DESCRIPTION_MANAGER , (tVoid**)&_pDescManager, NULL);
...
tResult rateLimiter::Init(tInitStage eStage, __exception) {
	RETURN_IF_FAILED(cFilter::Init(eStage, __exception_ptr));

	if (eStage == StageFirst) {
		cout << "Creating pins for rateLimiterfilter" << endl;
		RETURN_IF_FAILED(CreatePins(__exception_ptr));
	}
	if(eStage == StageGraphReady)
	{
		CreateDescriptors();
		cout << "rate limit filter started" << endl;
	}
...
tResult rateLimiter::Start(__exception) { return cFilter::Start(__exception_ptr); }

tResult rateLimiter::Stop(__exception) { return cFilter::Stop(__exception_ptr); }

tResult rateLimiter::Shutdown(tInitStage eStage, __exception) {
